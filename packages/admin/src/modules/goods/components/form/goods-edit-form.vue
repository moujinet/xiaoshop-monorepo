...
const form = defineModel('form', {
  type: Object as PropType<IGoodsFormData>,
  default: () => ({
...
const step = ref(1)
const steps = [
  { label: '1. 编辑基本信息', value: 1 },
  { label: '2. 编辑价格库存', value: 2 },
  { label: '3. 编辑商品详情', value: 3 },
  { label: '4. 发布商品', value: 4 },
]
...
const enableMultipleSku = computed(() => form.value.skuSpecs.length > 0)
  ...
  <FormCard :model="form" :loading="loading">
    <template v-if="step === 1">
      ...
      <FormGroup v-if="GOODS_TYPES.length > 1" title="商品类型">
        <GoodsTypeRadioCard v-model="form.type" />
      </FormGroup>
    ...
    <template #extra>
      <a-affix :offset-top="131">
        <div class="p4 bg-white select-none">
          <a-steps type="arrow" :current="step" small @change="(val) => step = val">
            <a-step v-for="s in steps" :key="s.value">
              {{ s.label }}
            </a-step>
          </a-steps>
        </div>
    ...
    <template #footer>
      <a-affix v-if="step !== 4" :offset-bottom="0">
        <div class="bg-white" flex="~ v-center between" border-t="~ solid $color-border-1" py="4">
          <a-button-group v-if="step <= 3" type="outline">
            <a-button v-if="step > 1" @click="handlePrevStep">
              <CommonIcon class="text-size-5 mr-1" name="ph:arrow-circle-left" :inline="false" />
              上一步
            </a-button>

            <a-button v-if="step < 3" type="outline" @click="handleNextStep">
              下一步
              <CommonIcon class="text-size-5 ml-1" name="ph:caret-circle-right" />
            </a-button>
          </a-button-group>
