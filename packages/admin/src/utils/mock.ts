...
export function defineMocks(definitions: IMockDefinition): IMockSetup {
  return () => {
    Object.entries(definitions).forEach(([url, fn]) => {
      // eslint-disable-next-line no-console
      console.log('🚦', `"${url}"`, 'api mocked')

      Mock.mock(url, ({ url, body }) => fn({
        url,
...
export function loadMocks() {
  ...
  Object.values(import.meta.glob<IMockSetup>('~/modules/**/mocks/*.ts', {
    eager: true,
    import: 'default',
  }),
  ).forEach(
