import { EnabledEnum, type IAssetGroup, type IAssetType, type IEnabled } from '@xiaoshop/schema'
...
export class AssetGroup implements IAssetGroup {
  ...
  @Column({ name: 'enable_compress', type: 'char', nullable: false, default: EnabledEnum.YES, comment: '启用图片压缩 (N:否 Y:是)' })
  enableCompress: IEnabled

  @Column({ name: 'enable_watermark', type: 'char', nullable: false, default: EnabledEnum.YES, comment: '启用图片水印 (N:否 Y:是)' })
  enableWatermark: IEnabled

  @Column({ name: 'enable_thumbnail', type: 'char', nullable: false, default: EnabledEnum.YES, comment: '启用图片缩略图 (N:否 Y:是)' })
