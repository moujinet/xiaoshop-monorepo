...
@Injectable()
export class MemberLevelUpService {
  constructor(
   ...
   * @returns 会员等级升级列表
   * @throws {FailedException} 获取会员等级升级列表失败
   */
  async findList(memberId: number): Promise<IMemberCardUpgradeList[]> {
    try {
      return await this.repo.find({
        memberId,
        method: MemberCardUpgradeMethod.LEVELUP,
      }).then(toMemberCardUpgradeList)
    }
    catch (e) {
      throw new FailedException('获取会员等级升级列表', e.message)
    }
  }
}
