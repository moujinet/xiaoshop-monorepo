...
export class LogisticAddressEntity {
  ...
  @Column({ name: 'is_default', type: 'tinyint', unsigned: true, default: 0, comment: '是否默认地址' })
  isDefault: YesOrNo

  @Column({ type: 'varchar', length: 16, nullable: false, default: '', comment: '联系人' })
  name: string

  @Column({ type: 'varchar', length: 16, nullable: false, default: '', comment: '手机号' })
  mobile: string

  @Column({ type: 'varchar', length: 16, nullable: false, default: '', comment: '座机号码' })
  landline: string

  @Column({ type: 'simple-json', default: null, comment: '所在城市' })
  location: ILocationPath

  @Column({ type: 'varchar', length: 200, nullable: false, default: '', comment: '详细地址' })
  address: string
