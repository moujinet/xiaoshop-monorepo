...
describe('System Organize Module - Position', () => {
  ...
  describe('Admin', () => {
    it('Create Position', async () => {
      await useRequest('post', '/system/position/create')
        .send({
          departmentId: 1,
          name: 'test',
        })
    ...
    it('Update Position', async () => {
      await useRequest('put', '/system/position/update')
        .query({ id: 1 })
        .send({
          departmentId: 2,
          name: 'test update',
    ...
    it('Fetch Position Detail', async () => {
      const { body } = await useRequest('get', '/system/position/detail')
        .query({ id: 1 })
        .expect(200)
    ...
    it('Fetch Position Pages', async () => {
      const { body } = await useRequest('get', '/system/position/pages')
        .expect(200)
    ...
    it('Fetch Position Dict List', async () => {
      const { body } = await useRequest('get', '/system/position/dict/list')
